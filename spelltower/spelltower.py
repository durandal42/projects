...
def all_plays_from(tower, x, y, trie_node, so_far=None, touched=None, min_length=3):
  ...
  if c == '.':
    return
  if c == '~':
    if 'Q' not in trie_node or 'U' not in trie_node['Q']:
      return
    so_far += 'QU'
...
def score_by_boggle(tower, play):
  ...
  length = len(play)
  if length in score_map:
    return score_map[length]
  return 11
...
scrabble_distribution = (['A'] * 9 + ['B'] * 2 + ['C'] * 2 + ['D'] * 4 + ['E'] * 12 + ['F'] * 2 + ['G'] * 3 + ['H'] * 2 + ['I'] * 9 + ['J'] * 1 + ['K'] * 1 + ['L'] * 4 + [
