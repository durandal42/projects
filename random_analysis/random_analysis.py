...
class ExploringRNG(RNG):
  ...
  def coin(self, weight=fractions.Fraction(1, 2)):
    result = (self.die(2) == 1)
    if weight < fractions.Fraction(1, 2):
      result = not result
...
def histogram(f, r=None):
  ...
  for i in itertools.count(1):
    ...
    mass += probability
    if is_power2(i):
      print('result #%d:\t%s (%s);\tprogress: %0.4f' %
            (i, outcome, probability, mass))
...
def summarize(f, r=None):
  h = histogram(f, r)
  print('summary:')
  for outcome, probability in sorted(h.items()):
    print('%s\t%5.2f%%\t%s' % (outcome, round(100*probability, 2), probability))
  try:
    ev = sum(outcome*probability for outcome, probability in h.items())
...
def black(rng):
  if rng.coin(fractions.Fraction(18, 38)):
    return 2
  else:
    return 0
