...
def restrict_targets(prev_guess, prev_score, scorer=no_dupes_scorer):
  if prev_guess:
    global remaining_targets
    remaining_targets = restrict(
        remaining_targets, prev_guess, prev_score, scorer)
...
def conservative_restricted_choice(prev_guesses, prev_scores, scorer=auto_scorer):
  ...
  for guess in remaining_targets:  # sorted(all_words):
    ...
    for target in remaining_targets:
      ...
      worst_score, worst_case = max(
          score_counts.iteritems(), key=lambda x: x[1])
...
#                guess, worst_score, worst_case)
  best_guess, best_worst_case = min(
      worst_case_by_guess.iteritems(), key=lambda x: x[1])
...
def ai_guesser(): play(targeter=no_target,
                       guesser=conservative_restricted_choice, scorer=user_score)
